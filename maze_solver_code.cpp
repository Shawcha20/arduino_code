...
{
    // making all motor to forward direction 
  ...
  while (1)
  {
  // ----------------------------------pid portion for the robot--------------------------------
    line_position = qtrrc.readLine(sensorValues); // reading sensor
    ...
    if (sensorValues[0] > threshold || sensorValues[6] > threshold)  // if we detect a turn left or right just return from the loopp 
    { 
      analogWrite(rightMotorPWM, 255);
      analogWrite(leftMotorPWM, 255);
      return;
    }
    // --------------------------if all the sensor are in the black line---------------------------
...
{
  while (1)
  { follow_line();   // after detecting a turn the follow line function will return here
    unsigned char found_left = 0;
    unsigned char found_straight = 0;
    unsigned char found_right = 0;
qtrrc.readLine(sensorValues);  // reading sensor values
    
    if (sensorValues[0] > threshold)   // found left
      found_left = 1;
else if (sensorValues[6] > threshold) // double checking the left turn for  avoiding issues
    {
     
      for (int i = 0; i < 100; i++)
      {
        qtrrc.readLine(sensorValues);  // reading the sensors for 100 time to check if we found an intersection if we do we go left
        if (sensorValues[0] > threshold)
        {
           found_left = 1;
           delay(310);    
           goto bailout;       // this will go to bail out function for checking if it is a cross intersection or not if cross it will go straight
         }
      }

      found_right = 1;         // else right will be found
    }
...
// for checking if the intersection is cross if it is cross it will go straight
bailout:
    qtrrc.readLine(sensorValues); // checking sensors
    // checking straight
    if (sensorValues[3] > threshold || sensorValues[4] > threshold|| sensorValues[2] > threshold) // checking if the middle sensors are in black line
      found_straight = 1;
...
break;
    ...
    unsigned char dir = select_turn(found_left, found_straight, found_right) // making a turn 

    // giving the values to the motor to turn 
    turn(dir);
